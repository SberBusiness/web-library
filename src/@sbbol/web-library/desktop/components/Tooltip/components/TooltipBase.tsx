...
import {
    ...
    ETooltipTypeName,
} from '@sbbol/web-library/desktop/components/Tooltip/enums';
import {Tip} from '@sbbol/web-library/desktop/components/Tooltip/Tip';
...
import {TooltipXButton} from '@sbbol/web-library/desktop/components/Tooltip/TooltipXButton';
import {
    ITooltipBaseProps,
    ITooltipBaseState,
    ITooltipBounds,
    ITooltipDomainSize,
    ITooltipSize,
} from '@sbbol/web-library/desktop/components/Tooltip/types';
import {addCrossPlatformStyle} from '@sbbol/web-library/desktop/components/Tooltip/utils/CSSPrefix';
import {
    axes,
    calcBounds,
    calcRelPos,
    ...
    equalCoords,
    pickZone,
} from '@sbbol/web-library/desktop/components/Tooltip/utils/Positioning';
import {isEqual} from '@sbbol/web-library/desktop/utils/isEqual';
import throttle from 'lodash.throttle';
import * as React from 'react';
import * as ReactDOM from 'react-dom';
import {IReorderConfig, reorderAndCheckChildrenDeprecated} from '@sbbol/web-library/desktop/utils/reactChildDeprecated';
import {ModalFocusManager} from '@sbbol/web-library/desktop/components/ModalFocusManager/ModalFocusManager';
...
export class TooltipBase extends React.Component<ITooltipBaseProps, ITooltipBaseState> {
    ...
    public render() {
        ...
        if (oldReact) {
            ...
            const tooltip = this.state.needRenderTooltip ? (
                <Portal node={this.renderContainer} key={2}>
                    {this.tooltipRender()}
                </Portal>
            ) : null;
    ...
    private trackTooltip = () => {
        ...
        /* Наблюдение за изменением границ на всех уровнях, при необходимсти - перерасчет. Привязка лэйаут монитора
             должна быть сделана сразу, т.к. перерасчет может вызывать рекурсивно измнение в границах. Поэтому если мы
              сделали одноразовый force-layout перед наблюдением, наши расчеты окончательной позиции могут быть неверными.
             */

        this.onWindowScroll = throttle(this.onWindowScroll, minScrollRefreshIntervalMs);
        this.onWindowResize = throttle(this.onWindowResize, minResizeRefreshIntervalMs);
    ...
    private tooltipRender: () => React.ReactElement = () => {
        ...
        return (
            <div className="cssClass[tooltipOverlay]">
                {this.props.modalFocusManager ? <ModalFocusManager>{tooltipEl}</ModalFocusManager> : tooltipEl}
            </div>
        );
    };
